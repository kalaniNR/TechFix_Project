...
namespace TECHFIX_PROJECT
{
    public partial class frmProductList : Form
    {
        ...
        public void LoadRecords()
        {
            ...
            cn.Open();
            cm = new SqlCommand("Select p.pcode,barcode,p.pdesc, b.brand, c.category, p.price, p.qty from tblProduct as p inner join tblBrand as b on b.id=p.bid inner join tblCategory as c on c.id=p.cid where c.category like '" + txtsearch.Text + "%'",cn);
            dr = cm.ExecuteReader();
            while (dr.Read())
            {
                i++;
